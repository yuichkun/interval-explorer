...
function list() {
  ...
  if (n === 1) {
    args = args.concat(args.map(function (n) {
      return n * -1;
    }));
...
function explore(arr, _k) {
  ...
  var cb = k_combinations(arr, _k); // console.log(cb);

  var stacked = cb.map(stackIntervals); // console.log(stacked);
